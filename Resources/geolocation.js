...
// CREATE ANNOTATIONS
//
	// Creates a new annotation point .createAnnotation actually places it
...
//
	...
	var regionAtlanta = {latitude:39.735673,longitude:-104.993806,animate:true,latitudeDelta:0.04, longitudeDelta:0.04};
	var regionSV = {latitude:39.735673,longitude:-104.993806,animate:true,latitudeDelta:0.04, longitudeDelta:0.04};

//
// CREATE MAP VIEW
//

	var presetAnnotations = [atlanta,office, mountainView];
...
//
	...
	var wireClickHandlers = function() 
	{
		...
		atl.addEventListener('click', function()
		{
			...
			Ti.API.error("CLICKED Jon's Pad");
		});

		sv.addEventListener('click', function() 
		{
			Ti.API.info('IN Office');
			// set location to sv
	...
	if (!isAndroid)
	{
		...
		activity.onCreateOptionsMenu = function(e) {
			...
			atl = menu.add({title : 'Jon\'s Shag Pad'});
			sv = menu.add({title : 'Arrae HQ'});
...
//
	...
	office.addEventListener('click', function(evt)
	{
		...
		var annotation = evt.source;
		var clicksource = evt.clicksource;
		Ti.API.info('office annotation click clicksource = ' + clicksource);
	});
